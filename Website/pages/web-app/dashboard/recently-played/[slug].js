...
export default class extends Component {
	...
	fetchData = async (id) => {
		...
		var videos = await get('videos/watch-history')
								...
								<div className="panel-group accordion" id="accordion" >
									{
										...
										<div className="panel panel-default">
											...
											<h5 className="panel-title click  mt-4 " data-toggle="collapse" data-parent={`#accordion`} href={`#collapsecourseWatchlist`} className="" aria-expanded="true">
											</h5>

											<ul id={`collapsecourseWatchlist`} className={`panel-collapse collapse in show`}>
												{this.state.videos && this.state.videos.length > 0 && this.state.videos.map((video, index) => {
													...
													return <li className={this.props.slug == video.id ? 'selected-list' : ''}>
														<div className="panel-heading pt-1" onClick={() => { this.fetchData(video.id) }}>
															<p className="panel-title click playlist mb-1" >
																<Link href={`/web-app/dashboard/recently-played/${video.id}`} >
																	<a className="row">
