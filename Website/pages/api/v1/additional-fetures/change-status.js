...
export default async function base(req, res) {
    ...
    params.data.is_active = params.data.is_active == true ? 1 : 0
        ...
        return trx(label)
            .modify(function (queryBuilder) {
                if (params.data.id.length == undefined) {
                    queryBuilder.where('id', params.data.id)
                } else {
                    queryBuilder.whereIn('id', params.data.id)
                }
            })
            .update('is_active', params.data.is_active);
    }).then(res => {
        return {
            id: res,
            error: null
