...
export default async function base(req, res) {
  ...
  if (count <= 0) {
    ...
    // Delete topper from database
    var topper = await knexConnection.transaction(async trx => {
      return trx('toppers').where('id', id).del();
    }).then(res => {
      return {
        id: res,
        error: null
  ...
  //
  let statusCode = topper && topper.id ? 200 : 422;
  let response = {
    success: topper && topper.id ? true : false,
    topper: topper,
  };
