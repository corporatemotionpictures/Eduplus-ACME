...
export default async function base(req, res) {
  ...
  let announcements = await knexConnection.transaction(async trx => {
    var query;

    query = trx.select(attributes).table('announcements').orderBy('announcements.position', orderBy);

    if ((!req.query.forList || req.query.listOnly) ) {
      query.where('announcements.is_active', 1)
    }
  ...
  //
  let statusCode = announcements.data ? 200 : 422;
  let response = {
    success: announcements.data ? true : false,
