...
export default async function base(req, res) {
  ...
  if (count <= 0) {
    ...
    // Delete ourTeam from database
    var ourTeam = await knexConnection.transaction(async trx => {
      return trx('our_teams').where('id', id).del();
    }).then(res => {
      return {
        id: res,
        error: null
  ...
  //
  let statusCode = ourTeam && ourTeam.id ? 200 : 422;
  let response = {
    success: ourTeam && ourTeam.id ? true : false,
    ourTeam: ourTeam,
  };
