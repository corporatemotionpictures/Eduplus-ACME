...
export default async function base(req, res) {
  ...
  let stores = await knexConnection.transaction(async trx => {
    var query;
    query = trx.select('stores.*').table('stores').orderBy('stores.position', orderBy);
  ...
  //
  let statusCode = stores.data ? 200 : 422;
  let response = {
    success: stores.data ? true : false,
    stores: stores.data,
