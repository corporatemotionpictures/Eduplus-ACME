...
export default async function base(req, res) {
  ...
  let position = await knexConnection.transaction(async trx => {
    return trx.select().table('stores').orderBy('position', 'DESC').first();
  ...
  let store = await knexConnection.transaction(async trx => {
    return trx.insert(params).into('stores');
  }).then(res => {
    return {
      id: res[0],
      error: null
  ...
  //
  let statusCode = store.id ? 200 : 422;
  let response = {
    success: store.id ? true : false,
    store: store,
    id: store.id
  };
