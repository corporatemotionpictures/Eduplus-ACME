...
export default async function base(req, res) {
  ...
  // Only ADMIN type of User allowed to insert tax
  ...
  if (count <= 0) {
    ...
    var tax = await knexConnection.transaction(async trx => {
      return trx('taxes').where('id', id).del();
    }).then(res => {
      return {
        id: res,
        error: null
  ...
  //
  let statusCode = tax && tax.id ? 200 : 422;
  let response = {
    success: tax && tax.id ? true : false,
    tax: tax,
  };
