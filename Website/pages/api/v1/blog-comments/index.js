...
export default async function base(req, res) {
  ...
  let comments = await knexConnection.transaction(async trx => {
    if (userID) {
      return trx.select('blog_comments.*', attributes).table('blog_comments')
        .innerJoin('users', 'blog_comments.user_id', 'users.id')
        .where('blog_comments.user_id', userID)
        .orderBy('blog_comments.id', orderBy).offset(offset);
    }
    else if (postID) {
      return trx.select('blog_comments.*', attributes).table('blog_comments')
        .innerJoin('users', 'blog_comments.user_id', 'users.id')
        .innerJoin('blogs', 'blog_comments.post_id', 'blogs.id')
