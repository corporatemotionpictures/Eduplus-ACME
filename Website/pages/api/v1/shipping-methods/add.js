...
export default async function base(req, res) {
  ...
  let position = await knexConnection.transaction(async trx => {
    return trx.select().table('shipping_methods').orderBy('position', 'DESC').first();
  ...
  let shippingMethod = await knexConnection.transaction(async trx => {
    return trx.insert(params).into('shipping_methods');
  }).then(res => {
    return {
      id: res[0],
      error: null
  ...
  //
  let statusCode = shippingMethod.id ? 200 : 422;
  let response = {
    success: shippingMethod.id ? true : false,
