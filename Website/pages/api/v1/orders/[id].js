...
export default async function base(req, res) {
  ...
  if (order.data) {
    ...
    if (order.data.type == 'PRODUCT') {
      ...
      order.data.total_membership_discount = order.data.carts.totalMembershipDiscount
      order.data.carts = order.data.carts.carts
    } else {
      order.data.carts = await fetchAll('membership-carts', { orderID: order.data.id, userID: order.data.user_id, noLog: true }, req.headers['x-auth-token'])
      order.data.carts = order.data.carts.carts
    }
