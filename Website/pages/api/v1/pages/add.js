...
export default async function base(req, res) {
  ...
  let position = await knexConnection.transaction(async trx => {
    return trx.select().table('pages').orderBy('position', 'DESC').first();
  ...
  let pageBySlug = await knexConnection.transaction(async trx => {
    return trx.select().table('pages').where('slug', params.slug).first();
  ...
  if (pageBySlug) {
    ...
    let page = await knexConnection.transaction(async trx => {
      return trx.insert(params).into('pages');
    }).then(res => {
      return {
        id: res[0],
        error: null
    ...
    if (page.id) {
      ...
      if (pageSections) {
        ...
        for (let i = 0; i < pageSections.length; i++) {
          ...
          // Insert page into database
          let pageSection = await knexConnection.transaction(async trx => {
          ...
          if (pageSection.id) {
            ...
            // Insert page into database
            let pageSectionData = await knexConnection.transaction(async trx => {
