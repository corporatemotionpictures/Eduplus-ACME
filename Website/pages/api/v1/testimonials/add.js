...
export default async function base(req, res) {
  ...
  // Only ADMIN type of User allowed to insert testimonial
  ...
  let schema = Joi.object({
    ...
    testimonial: Joi.string().allow(null),
    image: Joi.string()
  });
  ...
  let position = await knexConnection.transaction(async trx => {
    return trx.select().table('testimonials').orderBy('position', 'DESC').first();
  ...
  let testimonial = await knexConnection.transaction(async trx => {
    return trx.insert(params).into('testimonials');
  }).then(res => {
    return {
      id: res[0],
      error: null
  ...
  if (testimonial.id && (user.type == 'MANAGEMENT' || user.type == 'FACULTY')) {
    ...
    var notice = {
      ...
      redirect_url: `/dashboard/testimonials/view/${testimonial.id}`,
  ...
  //
  let statusCode = testimonial.id ? 200 : 422;
  let response = {
    success: testimonial.id ? true : false,
