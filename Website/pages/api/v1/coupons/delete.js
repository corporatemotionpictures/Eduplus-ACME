...
export default async function base(req, res) {
  ...
  if (count <= 0) {
    ...
    var coupon = await knexConnection.transaction(async trx => {
      return trx('coupons').where('id', id).del();
    }).then(res => {
      return {
        id: res,
        error: null
  ...
  //
  let statusCode = coupon && coupon.id ? 200 : 422;
  let response = {
    success: coupon && coupon.id ? true : false,
    coupon: coupon,
  };
