...
export default async function base(req, res) {
  ...
  let position = await knexConnection.transaction(async trx => {
    return trx.select().table('previous_year_question_papers').orderBy('position', 'DESC').first();
  ...
  let previousYearQuestion = await knexConnection.transaction(async trx => {
    return trx.insert(params).into('previous_year_question_papers');
  }).then(res => {
    return {
      id: res[0],
      error: null
  ...
  if (previousYearQuestion.id && (user.type == 'MANAGEMENT' || user.type == 'FACULTY' )) {
    ...
    var notice = {
      ...
      notification: `New PYQ Papers-${params.title} Added`,
      redirect_url: `/pyq_papers/${previousYearQuestion.id}`,
