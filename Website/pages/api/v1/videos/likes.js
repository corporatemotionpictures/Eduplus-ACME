...
export default async function base(req, res) {
    ...
    let likes = await knexConnection.transaction(async trx => {
        if (userID) {
    ...
    if (likes.data) {
        ...
        likes.data = await replaceUploadsArray(likes.data, 'user_image');

    }


    //
    let statusCode = likes.data ? 200 : 422;
    let response = {
        success: likes.data ? true : false,
        likes: likes.data,
        error: likes.error
    };
