...
export default async function (req, res) {
  ...
  if (!languagesAccess || Object.keys(languagesAccess).length > 0) {
    ...
    logs = await knexConnection.transaction(async trx => {
      return trx.select().table('logs')
        .where('user_id', user.id).andWhere('action', 'VIEW_VIDEO').orderBy('created_at', 'DESC');
    }).then(res => {
      return {
        data: res,
        error: null
    ...
    for (let i = 0; i < logs.data.length; i++) {
      ...
      if (!array.includes(payload.field_id)) {
        ...
        if (video) {
          ...
          video.thumbnail = await replaceUploads(video.thumbnail);
          ...
          video.watchDate = logs.data[i].created_at;
