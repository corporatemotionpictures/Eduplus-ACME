...
export default async function base(req, res) {
  ...
  // Only ADMIN type of User allowed to insert batch
  ...
  // Fatch batch From Database
  let batch = await knexConnection.transaction(async trx => {
    return trx.select().table('batches').where('referance_id', params.referance_id).first();
  }).then(res => {
    return {
      data: res,
      error: null
