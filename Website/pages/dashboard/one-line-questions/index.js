...
export default class extends Component {
  state = {
    ...
    storeFilter: {},
    imageLimits: {},
    baseTitle: define.oneLinerQuestions,
    modelTitle: 'question-banks',
    queryTitle: 'question_banks',
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      questionBank = await edit(`${this.state.modelTitle}`, data);
      message = `${this.state.baseTitle} Updated Successfully`

    } else {
      questionBank = await add(`${this.state.modelTitle}`, data);
      message = `${this.state.baseTitle} Added Successfully`
      added = true
    }

    // check Response
    if (questionBank.updated) {
      ...
      if (added) {
        ...
        let body = {
          ...
          body: `'Please open the app to check out the latest  ${this.state.baseTitle} ready`,
      ...
      if (questionBank.questionBank) {
        error = questionBank.questionBank.error
      }
      else if (questionBank.error) {
        error = questionBank.error.details ? questionBank.error.details[0].message :questionBank.error.details
      }
      toastr.error(error)
    }
  }
  ...
  // 
  render() {
    let data = this.state.questionBanks
    const columns = [
