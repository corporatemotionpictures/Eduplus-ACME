...
export default class extends Component {
  state = {
    ...
    storeFilter: {},
    imageLimits: {},
    baseTitle: 'Shipping Method',
    modelTitle: 'shipping-methods',
    queryTitle: 'shipping_methods',
  }
...
//     this.setState(data)
 ...
 buildAddModel = () => {
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      shippingMethod = await edit(this.state.modelTitle, data);
      message = `${this.state.baseTitle} Updated Successfully`
    } else {
      shippingMethod = await add(this.state.modelTitle, data);
      message = `${this.state.baseTitle} Added Successfully`
    }


    // check Response
    if (shippingMethod.updated) {
      ...
      if (shippingMethod.shippingMethod) {
        error = shippingMethod.shippingMethod.error
      }
      else if (shippingMethod.error) {
        error = shippingMethod.error.details ? shippingMethod.error.details[0].message : shippingMethod.error
      }
      toastr.error(error)
    }
  ...
  // 
  render() {
    let data = this.state.shippingMethods
    const columns = [
