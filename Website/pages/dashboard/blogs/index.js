...
export default class extends Component {
  ...
  buildAddModel = () => {
    ...
    let items = {
      ...
      'course_ids': {
        ...
        preFilters: { examID: examID ? examID : 'lock' },        name: 'course_ids',
        idSelector: 'id',
        view: 'name',
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      blog = await edit(this.state.modelTitle, data, 'image');
      message = `${this.state.baseTitle} Updated Successfully`

    } else {

      blog = await add(this.state.modelTitle, data, 'image');
    ...
    if (blog.updated) {
      ...
      let error;
      if (blog.blog) {
        error = blog.blog.error
      }
      else if (blog.error.details) {
        error = blog.error.details[0].message
      }
      else if (blog.error) {
        error = blog.error
      }
