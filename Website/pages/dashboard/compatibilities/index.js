...
export default class extends Component {
  state = {
    ...
    storeFilter: {},
    imageLimits: {},
    baseTitle: 'Compatibilitie',
    modelTitle: 'compatibilities',
    queryTitle: 'compatibilities',
  }
  ...
  buildAddModel = () => {
    ...
    this.setState({
      displayModelTitle: `Add Compatibility`
    })
  ...
  randerEditModal = (row) => {
    ...
    this.setState({
      displayModelTitle: `Edit Compatibility - ${row.title ? row.title : row.name}`
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      compatibily = await edit(this.state.modelTitle, data);
      message = `${this.state.baseTitle} Updated Successfully`
    } else {
      compatibily = await add(this.state.modelTitle, data);
      message = `${this.state.baseTitle} Added Successfully`
    }


    // check Response
    if (compatibily.updated) {
      ...
      let error;
      if (compatibily.compatibily) {
        error = compatibily.compatibily.error
      }
      else if (compatibily.error) {
        error = compatibily.error.details ? compatibily.error.details[0].message : compatibily.error
      }
      toastr.error(error)
    }
  ...
  // 
  render() {
    let data = this.state.compatibilities
    const columns = [
        ...
        <Widget
          ...
          {/* <Filter filterObjects={filterObjects}  filterOnChange ={true} onFilter={this.onFilter} /> */}
          {
            ...
            <Datatable
              ...
              data={this.state.compatibilities}
