...
export default class extends Component {
  state = {
    ...
    imageLimits: {},
    userModel: false,
    baseTitle: 'Self Assessment Result',
  ...
  fetchList = async () => {
    var data = await get('self-assessment-tests/result', { ...this.state.filters, 'forList': true });
    this.setState(data)
    this.setState({ fetching: false })
  }
        ...
        Cell: props => {
          return (
            <div className="relative">
              {props.row.original.user && <div className="capitalize userdata py-3 space-y-3" onMouseEnter={() => this.openPopup(props.row.original.user)}  >
                <span  >{props.row.original.user ? `${props.row.original.user.first_name} ${props.row.original.user.last_name}` : ''}</span>
              </div>}
              {
                !props.row.original.user && <h5>NA</h5>
              }
        ...
        Cell: props => {
          return (
            <div className="flex flex-wrap justify-start items-start" >
              ...
              <span className="pt-1">{props.row.original.finalMarks}</span>
            </div>
          )
        },
        ...
        Cell: props => {
          return (
            <div className="flex flex-wrap justify-start items-start" >
              ...
              <span className="pt-1">{props.row.original.totalMarks}</span>
            </div>
          )
        },
        ...
        Cell: props => {
          return (
            <a className="viewButton" title="View">
              {props.row.original.user && <span className="h-8 w-8 bg-blue-100 text-primary flex items-center justify-center rounded-full text-lg font-display font-bold">
