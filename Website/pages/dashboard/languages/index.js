...
export default class extends Component {
  state = {
    ...
    imageLimits: {},
    baseTitle: 'Language',
    modelTitle: 'languages',
    queryTitle: 'languages',
  }
...
//     this.setState(data)
 ...
 buildAddModel = () => {
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      language = await edit(this.state.modelTitle, data);
      message = `${this.state.baseTitle} Updated Successfully`
    } else {
      language = await add(this.state.modelTitle, data);
    ...
    if (language.updated) {
      ...
      if (language.language) {
        error = language.language.error
      }
      else if (language.error) {
        error = language.error.details ? language.error.details[0].message : language.error
      }
      toastr.error(error)
    }
