...
export default class extends Component {
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      subject = await edit(this.state.modelTitle, data, 'thumbnail');
      message = `${this.state.baseTitle} Updated Successfully`
    } else {
      subject = await add(this.state.modelTitle, data, 'thumbnail');
    ...
    if (subject.updated) {
      ...
      else if (subject.error.details) {
        error = subject.error.details[0].message
