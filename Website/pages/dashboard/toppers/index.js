...
export default class extends Component {
  ...
  buildAddModel = () => {
    ...
    let items = {
      ...
      'year': {
        label: 'year',
        error: { required: 'Please enter a valid year' },
        name: 'year',
        type: 'text',
        className: "sm:w-1/2",
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      topper = await edit(this.state.modelTitle, data, 'image');
      message = `${this.state.baseTitle} Updated Successfully`
    } else {
      topper = await add(this.state.modelTitle, data, 'image');
    ...
    if (topper.updated) {
      ...
      let error;
      if (topper.topper) {
        error = topper.topper.error
      }
      else if (topper.error) {
        error = topper.error.details ? topper.error.details[0].message : topper.error
      }
      toastr.error(error)
    }
    ...
    const columns = [
      ...
      },
      {
        ...
        Cell: props => {
          return (
            <div className="flex flex-wrap justify-start items-start" >
              <span className="pt-1">{props.row.original.year}</span>
            </div>
          )
        },
      ...
      },
      {
        ...
        Cell: props => {
          return (
            <div className="flex flex-wrap justify-start items-start" >
              <span className="pt-1">{props.row.original.air}</span>
            </div>
          )
        },
