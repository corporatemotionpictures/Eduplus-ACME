...
export default class extends Component {
  ...
  onStoreSubmit = async (data) => {
    ...
    if ('id' in data) {
      chapter = await edit(this.state.modelTitle, data);
      message = `${this.state.baseTitle} Updated Successfully`
    } else {
      chapter = await add(this.state.modelTitle, data);
    ...
    if (chapter.updated) {
      ...
      let error;
      if (chapter.chapter) {
        error = chapter.chapter.error
      }
      else if (chapter.error) {
        error = chapter.error.details ? chapter.error.details[0].message : chapter.error
      }
      toastr.error(error)
    }
