        ...
        return (
            <TouchableOpacity onPress={() => {
                    AsyncStorage.setItem("last_seen_chapter", item.id.toString()).then(() => {
                     this.props.navigation.navigate("Lecture", { item: item })
                    })
            }}>
        ...
        return this.state.loading ? <Loader /> : (
            <View style={styles.container}>
                ...
                <View style={{ backgroundColor: 'white', alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: heightPercentageToDP(3), color: 'white', position: 'absolute', bottom: 28, left: 10 }} >{this.state.item.name}</Text>
        <Text style={{ fontSize: heightPercentageToDP(2), color: 'white', position: 'absolute', bottom: 10, left: 10 }} >Chapters in {this.state.item.name}</Text>
