...
export default class GeniqueFreeCourses extends Component {
    constructor(props) {
        super(props);
   ...
   async componentWillMount() {
        ...
        AsyncStorage.getItem("user_token").then(token => {
            ...
            fetchSingleCourses(token, this.state.offset,null,"Free")
                .then(res => {
                    if (res.success) {
                        this.setState({
                            loading: false,
    ...
    render() {
        return this.state.loading ? <Loader /> : (
            <View style={styles.container}>
                <Header backIcon onbackIconPress ={() => this.props.navigation.goBack()} title={"Free Courses"} rightIcons={[<MaterialIcons onPress={() => this.props.navigation.navigate("WatchList")} name={"bookmark" } size={20} color={"white"} style={{ right: 18, position: 'absolute' }} />]} />
                <View style={{ backgroundColor: 'white', alignItems: 'center' }}>
                    ...
                    <LinearGradient colors={["transparent", "transparent"]} style={{ position: 'absolute', top: 0, left: 0, bottom: 0, right: 0 }} >
                        {/* <Text style={{ fontSize: heightPercentageToDP(3), color: 'white', position: 'absolute', bottom: 15, left: 10, width: widthPercentageToDP(80) }} >
                            Simply Physix Free Courses
                        </Text> */}
                    </LinearGradient>
                ...
                {/* <HeadingText text={"Free Courses"} /> */}
                {this.state.courses.length == 0 ? <BlankError text="No Free Courses found" /> :
                    <FlatList
