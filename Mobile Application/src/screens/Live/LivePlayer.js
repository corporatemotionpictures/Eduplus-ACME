...
import { sendVideoLogs, addVideoComments, fetchVideoComments, BASE_URL, fetchVideo, fetchVideoLikes, addVideoLikes, removeVideoLikes, addVideoWatchList, removeWatchLists, fetchChapterSubjectWise, fetchVideos, fetchUserDetails, fetchSingleVideo, fetchSingleEvent } from '../../utils/configs';
...
export default class LivePlayer extends Component {
  ...
  componentWillMount() {
    ...
    AsyncStorage.getItem("user_token").then((token) => {
      ...
      fetchSingleEvent(this.state.videoData.id, token, "1080p").then(data => {
        console.log(data.event.embed_chat_url);
        if (data.success) {
          this.setState({
            videoData: data.event,
            videoUrl: data.event.videoUrl,
            play: true,
            status: data.event.live_event
          })
      ...
      <View style={[styles.BG_STYLE, { padding: 0, }]}>
        ...
        <View style={{ width: this.state.videoWidth, height: this.state.videoHeight }}>
          <TouchableOpacity onPress={this.handleOverlay} activeOpacity={1} style={{ width: this.state.videoWidth, height: this.state.videoHeight }}>
            ...
            {
              this.state.status ? null : (this.state.videoData.videoUrl == undefined ? null : <Animatable.View duration={100} animation={this.state.overlay ? "fadeIn" : "fadeOut"} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 40, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: "center", alignItems: 'center', padding: 10, flexDirection: 'row' }}>
