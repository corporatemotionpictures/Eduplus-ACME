...
import { WIDTH, LIGHTGREY, BLUE, NEWBLUE } from '../../utils/utils'
...
export default function CourseSubject(props) {
  ...
  const renderItems = ({ item, index }) => {
    return (
      <View>
        <TouchableOpacity onPress={() => props.navigation.navigate("VideoList",{item:item})} style={{ marginHorizontal: 10, borderBottomColor: "#ececec", borderBottomWidth: .5, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: heightPercentageToDP(2), }} >
          <Image source={require("../../../assets/online-class.png")} style={{ height: heightPercentageToDP(3), width: heightPercentageToDP(3) }} />
          <View>
            <Text numberOfLines={2} style={{ color: 'black', fontSize:heightPercentageToDP(2), width: widthPercentageToDP(75), fontFamily: 'Roboto-Regular' }}>{item.name}</Text>
