...
renderChapters = ({item,index}) => {
    return item.locked ? null : (
        <TouchableOpacity onPress={() => this.props.navigation.navigate("PreviousYear",{item:item})} style={{height:80,margin:10,borderBottomColor:LIGHTGREY,borderBottomWidth:.5,flexDirection:'row',justifyContent:'space-between',}} >
                  <View >
                      <Text numberOfLines={2} style={{color:'grey',fontSize:15,width: 280,}}>{item.name}</Text>
    <Text numberOfLines={2} style={{backgroundColor:GREEN,padding:5,paddingHorizontal:8,color:'white',borderRadius:4,textAlign:'center',width:120,marginTop:5,fontSize:10}}  >{item.subject_name}</Text>
                  </View>
        ...
        <Background/>
         ...
         (this.state.chapters.length !== 0 ?  
             <View>
              ...
              : 
              <BlankError text="Documents not available" />
              )}
                </ScrollView>   
      </View>
    );
