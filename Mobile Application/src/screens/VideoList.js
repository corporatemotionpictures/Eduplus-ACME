...
import { styles, Header, LIGHTGREY, WIDTH, BLUE, GREEN } from '../utils/utils';
...
export default class VideoList extends Component {
  ...
  componentDidMount() {
    ...
    AsyncStorage.getItem("user_token").then((token) => {
      ...
      fetchVideo(this.state.chapterID, this.state.getproductid, token, this.state.offset).then((data) => {
        this.setState({
          videos: data.videos,
          loading: false,
          token: token
  ...
  fetchMore = () => {
    ...
    console.log(offset)
    fetchVideo(this.state.chapterID, this.state.getproductid, this.state.token, offset).then(data => {
    ...
    return (
      <View style={styles.BG_STYLE}>
        ...
        {this.state.loading ? <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}><Loader /></View> : (this.state.videos.length !== 0 ?
          ...
          /> : <BlankError text="Videos not available" />)}

        <View style={{ position: 'absolute', bottom: hp("2"), left: 0, right: 0, height: hp("10"), justifyContent: 'center', alignItems: "center" }}>
          <View style={{ backgroundColor: GREEN, padding: hp("1"), width: "90%", borderRadius: wp("1"), alignItems: 'center', flexDirection: 'row', }}>
            <Foundation name="alert" size={hp("2.4")} color="white" />
            <Text style={{ color: 'white', fontSize: hp("1.5"), marginLeft: wp("2") }}>
