...
import { View, Image, TouchableOpacity, ScrollView, Modal, ImageBackground, StyleSheet, AsyncStorage, ActivityIndicator, FlatList, Alert, TextInput, Picker, Style, TouchableHighlight } from 'react-native';
...
import { BG_COLOR, ORANGE, BLUE, Header, LIGHTGREY, GREEN, WIDTH, LIGHT_BLUE, NEW_GRAD1, WHITE, NEW_GRAD2, HEIGHT, NEWBLUE, BLUELIGHT, LIGHT_GREEN } from '../utils/utils';
import { Ionicons, MaterialIcons, AntDesign, Entypo, MaterialCommunityIcons } from "@expo/vector-icons"
import { Block, Icon } from 'galio-framework';
import { BASE_URL, fetchCourses, fetchSingleCourses, fetchCartList, fetchSubjects, fetchSubject, fetchProductDetails, addToCart, addToCartFree } from '../utils/configs';
...
export default class CourseBuys extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillMount = async () => {
    ...
    AsyncStorage.getItem("user_token").then(token => {
      ...
      fetchProductDetails(this.state.item.id, token)
        .then(res => {
          if (res.success) {
            console.log(res)
            this.setState({
  ...
  renderFaculty = ({ item, index }) => {
    ...
    return (
      <View style={{ flex: 1, width: widthPercentageToDP(100), justifyContent: 'center', alignItems: 'center', alignContent: 'center' }}>
        <View style={{ flexDirection: 'row', marginTop: 10, justifyContent: 'center', alignItems: 'center', width: widthPercentageToDP(45), height: 40, borderColor: '#e5e7eb', borderRadius: widthPercentageToDP(1), borderWidth: heightPercentageToDP(.2), elevation: 0, }}>
          ...
          <Text style={{ flex: 1, color: 'black', fontSize: 12, fontFamily: 'Roboto-Regular', marginLeft: heightPercentageToDP(2) }} numberOfLines={1} adjustsFontSizeToFit>
  ...
  addToFree = async () => {
    ...
    this.setState({ addtoloading: true })
    AsyncStorage.getItem("user_token").then(token => {
      addToCartFree(token, this.state.productdetails.id, 0, 0, this.state.selectedAttributes, "ONLINE").then(res => {
        if (res.success) {
          //this.fetchcartLength();
          console.log("ADDED CART RESPONSE")
          console.log(res)
          this.props.navigation.push("Main")
          Alert.alert("Product is Activated")
          //this.componentWillMount();
          this.Show_Custom_Alert();
          this.setState({
            loading: false,
  ...
  render() {
    ...
    return this.state.loading ? <Loader /> : (
      <View style={{ flex: 1 }}>
        ...
        <ScrollView>
          {/* <View style={{ flexDirection: 'row', justifyContent: 'flex-end', position: 'absolute', alignSelf: 'flex-end', alignItems: 'flex-end', top: heightPercentageToDP(2)}}>
          <AntDesign onPress={() => this.props.navigation.navigate("CartList")} name="shoppingcart" size={27} color="white" style={{ marginRight: 15, marginTop: 15 }} />
          <Text style={{ fontSize: heightPercentageToDP(2), justifyContent: 'center', alignContent: 'center', alignItems: 'center', alignSelf: 'center', color: 'white', backgroundColor: ORANGE, height: 20, width: 20, borderRadius: 10, position: 'absolute', left: 15, top: 6, textAlign: 'center' }}>{this.state.cartlength}</Text>
        </View> */}

          <View>
            <ImageBackground source={{ uri: `${BASE_URL}${this.state.item.cover_image}` }} style={{ height: heightPercentageToDP(20), width: widthPercentageToDP(100) }} >
              <LinearGradient colors={[BG_COLOR, BLUELIGHT]} start={[0.1, 0.1]} style={{ position: 'absolute', left: 0, bottom: 0, right: 0, opacity: .9, height: heightPercentageToDP(20), }} >
                <Text numberOfLines={2} style={{ color: WHITE, marginHorizontal: heightPercentageToDP(2), marginTop: heightPercentageToDP(2), fontSize: heightPercentageToDP(3), fontWeight: 'bold' }}>{this.state.item.name}</Text>
                <Block marginLeft={heightPercentageToDP(2)} color={LIGHTGREY}>
                  <Text style={{ fontSize: heightPercentageToDP(2), color: 'white' }}>{this.state.item.model}</Text>

                  <Star score={this.state.item.average_review} style={styles.starStyle} />
            ...
            <View>
              ...
              <View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  {/* <Text numberOfLines={1} style={{ color: LIGHTGREY, marginHorizontal: heightPercentageToDP(2), marginTop: heightPercentageToDP(1), fontSize: heightPercentageToDP(3), fontWeight: 'bold' }}>{this.state.item.name}</Text> */}
                ...
                <ScrollView crollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: heightPercentageToDP(3), marginLeft: heightPercentageToDP(2) }}>
                    {this.state.productdetails == undefined ? null :
                      (
                        this.state.productdetails.exams.map((exam, index) => {
                          return (
                            <Text style={{ color: '#07A1E8', backgroundColor: '#03A2E829', paddingVertical: heightPercentageToDP(.6), paddingHorizontal: heightPercentageToDP(2), borderWidth: 1, borderColor: "#07A1E8", borderRadius: 4, marginHorizontal: 2, fontSize: heightPercentageToDP(1.8) }}>{exam.name}</Text>
                          )
                        })
                      )
                    }
                    {this.state.productdetails == undefined ? null :
                      (
                        this.state.productdetails.courses.map((course, index) => {
                          return (
                            <Text style={{ color: '#07A1E8', backgroundColor: '#03A2E829', paddingVertical: heightPercentageToDP(.6), paddingHorizontal: heightPercentageToDP(2), borderWidth: 1, borderColor: "#07A1E8", borderRadius: 4, marginHorizontal: 2, fontSize: heightPercentageToDP(1.8) }}>{course.name}</Text>
                          )
                        })
                      )
                    }
              ...
              {this.state.item.description == undefined ? null
                :
                <View style={{ flex: 1 }} >
                  {
                    this.state.tab == "1" ?
                      <Text onPress={() => this.setState({ seeMore: !this.state.seeMore })} style={{ fontSize: heightPercentageToDP(1.5), color: 'black', textAlign: 'justify', marginHorizontal: 10, marginVertical: 7, marginBottom: 10, width: widthPercentageToDP(95), lineHeight: 24 }} numberOfLines={this.state.seeMore ? null : 3} >{this.state.seeMore ? <Text numberOfLines={2} style={{}}>{this.state.item.description.replace(/<\/?[^>]+(>|$)/g, "")}{'\n'}<Text style={{ color: NEW_GRAD2, fontFamily: 'Roboto-Bold' }}>...Show less</Text></Text> : <Text numberOfLines={2} style={{}}>{this.state.item.description.slice(0, 160).replace(/<\/?[^>]+(>|$)/g, "")}...<Text style={{ color: NEW_GRAD2, fontFamily: 'Roboto-Bold' }}>Show More</Text></Text>}</Text>
                      :
                      <View>
                      </View>
                  }
                          ...
                          <View style={styles.row}>
                            ...
                            <View>
                              <View style={styles.nameContainer}>
                                <Text style={styles.nameTxt} numberOfLines={2} ellipsizeMode="tail">{item.title}</Text>
                                <Text style={styles.nameTxt2} numberOfLines={2}>{item.value}</Text>
                              </View>
                            </View>
                          </View>
                        </TouchableOpacity>}
        ...
        {this.state.productdetails == undefined || this.state.productdetails.AddedToCart == undefined ? null :
          <View>
            {this.state.productdetails.AddedToCart == true ? (
              ...
              <TouchableHighlight onPress={() => this.RBSheet.open()} style={{ height: heightPercentageToDP(7), width: widthPercentageToDP(95), bottom: 1, justifyContent: 'center', alignItems: 'center', alignContent: 'center', alignSelf: 'center', marginBottom: heightPercentageToDP(2) }} >
                <LinearGradient colors={[LIGHT_GREEN, LIGHT_GREEN]} start={[0.1, 0.1]} style={{ flex: 1, position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, justifyContent: 'center', alignItems: 'center', borderRadius: 12 }} >
                  <View style={{ flexDirection: 'row' }}>
        ...
        >
          ...
          {this.state.addtoloading ?
            <TouchableOpacity activeOpacity={.7}>
              <LinearGradient colors={[LIGHT_BLUE, BLUE]} start={{ x: -.1, y: 0.2 }} end={{ x: 1, y: 0 }} style={{ position: 'absolute', bottom: 20, right: 20, height: 60, width: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center', backgroundColor: NEW_GRAD2 }} >
                <ActivityIndicator color="white" />
              </LinearGradient>
            </TouchableOpacity> :
            <LinearGradient colors={[LIGHT_BLUE, BLUE]} start={{ x: -.1, y: 0.2 }} end={{ x: 1, y: 0 }} style={{ position: 'absolute', bottom: 20, right: 20, height: 60, width: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center', backgroundColor: NEW_GRAD2 }} >
              {this.state.productdetails == undefined || this.state.productdetails.finalAmount == 0 ?
                <View style={{}}>
                  <TouchableOpacity onPress={() => { this.addToFree() }}  >
...
const styles = StyleSheet.create({
  ...
  nameTxt: {
    marginTop: 10, marginLeft: 15, width: 170, marginBottom: 10, fontSize: heightPercentageToDP(1.5), marginLeft: heightPercentageToDP(1), color: 'grey', textAlign: 'left', margin: 10
  },
  nameTxt2: {
    marginTop: 10, marginLeft: 15, width: 170, marginBottom: 10, marginLeft: heightPercentageToDP(-4), fontSize: heightPercentageToDP(1.5), color: 'black', textAlign: 'left'
  },
  mblTxt: {
    color: 'black',
